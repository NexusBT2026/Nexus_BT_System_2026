        ...
        for idx in data.index:
            if idx in signal_map:
                ...
                try:
                    ...
                    if isinstance(ind_idx, slice):
                        ind_idx = ind_idx.start
                    elif isinstance(ind_idx, np.ndarray):
                        ind_idx = int(ind_idx[0])
                    else:
                        ind_idx = int(ind_idx)
                    
                    if ind_idx >= len(indicators['rsi']):
                        ind_idx = len(indicators['rsi']) - 1
                    
                    rsi_val = indicators['rsi'].iloc[ind_idx] if ind_idx < len(indicators['rsi']) else None
                except:
                    rsi_val = None
