...
class GateioOHLCVDataSource:
    """Fetches OHLCV data from Gate.io using CCXT - uses SWAP (perpetuals) for MASSIVE liquidity"""
    
    def __init__(self, api_key: Optional[str] = None, api_secret: Optional[str] = None, market_type: str = 'swap'):
        """Initialize Gate.io CCXT client for swap (perpetuals) - 36,000x higher volume than spot!"""
        ...
        if not self.api_key or not self.api_secret:
            try:
                ...
                logger.warning(f'Could not load Gate.io API credentials from central config loader: {e}')
    ...
    def fetch_historical_data(
        ...
        try:
            ...
            ohlcv = self.exchange.fetch_ohlcv(
                symbol=symbol,
                timeframe=timeframe,
                limit=min(limit, 1000),  # Gate.io max is 1000
                since=since
            )
    ...
    def validate_symbol(self, symbol: str) -> bool:
        """Check if symbol is available on Gate.io"""
        try:
            # Convert symbol format if needed
            if '/' not in symbol:
                if self.market_type == 'swap':
