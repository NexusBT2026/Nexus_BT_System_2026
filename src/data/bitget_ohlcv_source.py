...
class BitgetOHLCVDataSource:
    """Fetches OHLCV data from Bitget using CCXT - uses SWAP (perpetuals) for high liquidity"""
    
    def __init__(self, api_key: Optional[str] = None, api_secret: Optional[str] = None, market_type: str = 'swap'):
        """Initialize Bitget CCXT client for swap (perpetuals) - 6x higher volume than spot"""
        ...
        if not self.api_key or not self.api_secret:
            try:
                ...
                logger.warning(f'Could not load Bitget API credentials from central config loader: {e}')
    ...
    def get_available_timeframes(self) -> list:
        ...
        return ['1m', '3m', '5m', '15m', '30m', '1h', '4h', '6h', '12h', '1d', '3d', '1w', '1M']
    
    def validate_symbol(self, symbol: str) -> bool:
        """Check if symbol is available on Bitget"""
        try:
            # Convert symbol format if needed
            if '/' not in symbol:
                if self.market_type == 'swap':
